    ...
    <div className={mobile ? 'flex md:hidden flex-col gap-y-2' : "hidden md:flex justify-center gap-x-6 items-center"}>
      {
        LINKS.map((link, index) =>
          link.type === "normal" ? (
            <a key={`link:${index}:${link.href}`} href={link.href} className={`${mobile ? 'text-center py-1 text-xl' : 'text-lg'} ${pathname === link.href ? 'text-blue-400' : 'text-white hover:text-blue-400'}`}>{link.label}</a>
          ) : (
