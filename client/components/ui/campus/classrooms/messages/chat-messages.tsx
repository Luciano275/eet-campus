...
export default function ClassroomChatMessages(
  {classroomId, socketUrl, apiUrl, userId}
  : {
    classroomId: string;
    socketUrl: string;
    apiUrl: string;
        ...
        data?.pages?.map((group, i) => (
          <Fragment key={`classroom:${i}`}>
              {group.messages.map((msg, index) => (
                <div className="flex gap-2 items-start py-4 border-b border-base-300" key={`message:${msg.id}:${index}`}>
                  ...
                  <div className="flex grow flex-col gap-1">
                    <p className={`text-sm ${theme === 'dark' ? 'text-neutral-500' : 'text-neutral-400'}`}>
                      <TimeAgo formatter={buildFormatter(spanishStrings)} date={new Date(msg.created_at)} />
                    </p>
                    <h2 className="text-xl text-base-content justify-between flex gap-2 items-center">
                      <span>{msg.owner.name}</span>
                      { userId === msg.owner.id && <DeleteMessage /> }
                    </h2>
